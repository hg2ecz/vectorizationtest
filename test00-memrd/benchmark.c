...
int main() {
	...
	for (int i=0; i<iter; i++) memrd_c1(in, dnum);
	...
	clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &gstart);
	for (int i=0; i<iter; i++) memrd_c2(in, dnum);
	clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &gend);
	eltime_ms[et++] = 1000.0*(gend.tv_sec - gstart.tv_sec) + (gend.tv_nsec - gstart.tv_nsec)/1000000.;

	clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &gstart);
	for (int i=0; i<iter; i++) memrd_v(in, dnum);
	clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &gend);
	eltime_ms[et++] = 1000.0*(gend.tv_sec - gstart.tv_sec) + (gend.tv_nsec - gstart.tv_nsec)/1000000.;

	if (print_enable) printf("arrlen: %8d (iter: %5d), eltime float: %f, float2: %f, vector: %f\n",
	dnum, iter, eltime_ms[0]/ITERNUM, eltime_ms[1]/ITERNUM, eltime_ms[2]/ITERNUM);
